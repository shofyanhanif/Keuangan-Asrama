  ...
  <div class="modal fade" id="strukLangsung<?php echo $row2['ID_SANTRI'] ?>" tabindex="-1" aria-labelledby="strukLangsungLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body" style="font-size: 12px; padding: 5px">
          <div class="table-responsive">
            <table class="table table-striped text-gray-700" id="tabel-langsung">
              <?php
              $idPembayaran = $row2['ID_PEMBAYARAN'];
              $idPetugas = substr($idPembayaran, 4, 1);
              $query = "SELECT * FROM petugas WHERE ID_PETUGAS = '$idPetugas'";
              $result = mysqli_query($koneksi, $query);

              if (mysqli_num_rows($result) > 0) {
                $data2 = mysqli_fetch_assoc($result);
                $namaPetugas = $data2['NAMA_PETUGAS'];
              } else {
                $namaPetugas = "Nama tidak ditemukan!";
              }
              ?>
              <tr>
                <th>Petugas</th>
