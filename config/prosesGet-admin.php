  ...
  switch ($_GET['get']) {
    ...
    case 'kota':
      if (isset($_POST['idProvinsi'])) {
        ...
        // Mengirim data kota sebagai response
        echo $dataKota;
      }
    ...
    case 'alamatSantri':
      // Mendapatkan nilai id_santri dari permintaan POST
      ...
      // Mengirimkan data santri sebagai respons AJAX dalam format JSON
    ...
    case 'kamarSantri':
      ...
      // Proses get tagihan berdasarkan id_santri
    case 'tagihan':
      $idSantri = $_POST['id_santri'];

      $query = "SELECT * FROM tagihan WHERE ID_SANTRI = '$idSantri' AND STATUS = 'Tunggakan'";
      $result = mysqli_query($koneksi, $query);

      // Menghasilkan HTML untuk setiap baris data tagihan
      $output = '';
      if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          $output .= '<tr>';
          $output .= '<td>' . $no++ . '</td>';
          $output .= '<td>' . $row['ID_SANTRI'] . '</td>';
          $output .= '<td>' . $row['TAHUN'] . '</td>';
          $output .= '<td>' . $row['BULAN'] . '</td>';
          $output .= '<td>' . $row['KATEGORI'] . '</td>';
          $output .= '<td>' . $row['NOMINAL'] . '</td>';
          $output .= '</tr>';
        }
      } else {
        $output .= '<tr>';
        $output .= '<td colspan="6">Data tagihan tidak ditemukan</td>';
        $output .= '</tr>';
      }
